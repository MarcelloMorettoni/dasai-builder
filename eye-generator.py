...
BG = 0
FG = 255
LOGICAL_STEPS = 48  # shape quality (not speed)
...
# ===============================
# VIDEO GENERATION (SPEED CONTROLLED)
# ===============================
def generate_animation(start, end, duration_sec, fps):
    ...
    for i in range(total_frames):
        t = i / (total_frames - 1)  # exact 0 → 1
        p = interpolate_params(start, end, t)
...
# GRADIO CALLBACK
# ===============================
def build_all(duration, fps, *vals):
    keys = [
    ...
    video = generate_animation(start, end, duration, fps)
...
with gr.Blocks(title="🧠 Eye Transition Designer") as demo:
    ...
    with gr.Row():
        start_controls = controls("START (Idle)")
        end_controls = controls("END (Emotion)")
    ...
    btn.click(
        build_all,
        inputs=[duration, fps] + start_controls + end_controls,
